...
def show_operation(conversation, parse_text, i, n_features_to_show=float("+inf"), **kwargs):
    ...
    if len(data) == 1:
        ...
        """
        return_string = f"{intro_text} the feature values are on average:\n\n"
        for i, feature_name in enumerate(data.columns):
        ...
        return_string = f"{intro_text} the instance id's are:\n\n"
        return_string += instance_ids
        return_string += "\n\nWhich one do you want to see?\n\n"

    # If we've written additional info to this string
    if len(rest_of_info_string) > init_len:
        return_string += "\n\nI've truncated this instance to be concise. Let me know if you"
        return_string += " want to see the rest of it.\n\n"
